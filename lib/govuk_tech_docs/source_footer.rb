module GovukTechDocs
  ...
  class SourceUrls
    ...
    def report_issue_url
      "#{repo_url}/issues/new?labels=bug&title=Problem with '#{current_page.data.title}'&body=Problem with '#{current_page.data.title}' (#{config[:tech_docs][:host]}#{current_page.url})"
    end
